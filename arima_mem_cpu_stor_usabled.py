...
#data = pd.read_excel('mem_usabled_days_2017.xls')
#
...
data_analysis = data['cpu_usabled'];
#data_analysis = data['stor_usabled'];
...
# 当n>某一个值时，x(t)与x(t-n)的相关性总为0.AM模型仅通过时间序列变量的自身历史观测值来反映有关因素对预测目标的影响和作用，
# 步骤模型变量相对独立的假设条件约束，所构成的模型可以消除普通回退预测方法中由于自变量选择、
# 多重共线性等造成的困难。即AM模型中x(t)与x(t-1)的相关性随着时间的推移变得越来越小。这个差别要好好利用起来。


# 时间序列x(t)滞后k阶的样本自相关系数（ACF）和滞后k期的情况下样本偏自相关系数（PACF）

# AR模型的ACF和PACF:AR（p）模型：自相关系数拖尾，偏自相关系数p阶截尾。
#	AR的ACF为拖尾序列，即无论滞后期k取多大，ACF的计算值均与其1到p阶滞后的自相关函数有关。 
#	AR的PACF为截尾序列，即当滞后期k>p时PACF=0的现象。 

# MA模型的ACF和PACF： MA（q）模型：自相关系数q阶截尾，偏自相关系数拖尾。
#	MA的ACF为截尾序列，即当滞后期k>p时PACF=0的现象。
#	MA的PACF为拖尾序列，即无论滞后期k取多大，PACF的计算值均与其1到p阶滞后的自相关函数有关。 
...
fig = plt.figure(figsize = fig_size )
ax1 = fig.add_subplot(211)
fig = sm.graphics.tsa.plot_acf(diff_data,lags = lagnum,ax=ax1)
ax2 = fig.add_subplot(212)
fig = sm.graphics.tsa.plot_pacf(diff_data,lags = lagnum,ax=ax2)
plt.show() # 在Scala IDE要输入这个命令才能显示图片
'''
